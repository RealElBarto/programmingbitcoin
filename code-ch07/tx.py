...
class Tx:
    ...
    def sig_hash(self, input_index):
        ...
        for tx_in_number in range(len(self.tx_ins)):
            if tx_in_number == input_index:
                s += TxIn(
                    prev_tx=self.tx_ins[tx_in_number].prev_tx,
                    prev_index=self.tx_ins[tx_in_number].prev_index,
                    # the previous tx's ScriptPubkey is the ScriptSig
                    script_sig=self.tx_ins[tx_in_number].script_pubkey(self.testnet),
                    sequence=self.tx_ins[tx_in_number].sequence
                    ).serialize()
            else:
                s += TxIn(
                    prev_tx=self.tx_ins[tx_in_number].prev_tx,
        ...
        """
        for count, tx_in in enumerate(self.tx_ins):
