...
class Block:
    ...
    def serialize(self):
        ...
        block_header = int_to_little_endian(self.version, 4)
        # prev_block - 32 bytes, little endian
        block_header += self.prev_block[::-1]
        # merkle_root - 32 bytes, little endian
        block_header += self.merkle_root[::-1]
        # timestamp - 4 bytes, little endian
        block_header += int_to_little_endian(self.timestamp, 4)
        # bits - 4 bytes
        block_header += self.bits
        # nonce - 4 bytes
        block_header += self.nonce
        return block_header
    ...
    def check_pow(self):
        ...
        sha = self.hash()
        proof = little_endian_to_int(sha[::-1])
